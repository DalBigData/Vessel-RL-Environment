...
"""
from server.math.size_2d import *
from server.math.segment_2d import *
from server.math.region_2d import *
...
class Rect2D(Region2D):
    ...
    def intersection(self, *args):  # , **kwargs):):
        if len(args) == 1 and isinstance(args[0], Line2D):
            ...
            res = self.leftEdge().intersection(line)
            if n_sol < 2 and res.is_valid() and top_y <= res._y <= bottom_y:
                t_sol[n_sol] = res
                n_sol += 1
            res = self.rightEdge().intersection(line)
            if n_sol < 2 and res.is_valid() and top_y <= res._y <= bottom_y:
                t_sol[n_sol] = res
                n_sol += 1
            res = self.topEdge().intersection(line)
            if n_sol < 2 and res.is_valid() and left_x <= res._x <= right_x:
                t_sol[n_sol] = res
                n_sol += 1
            res = self.bottomEdge().intersection(line)
            if n_sol < 2 and res.is_valid() and left_x <= res._x <= right_x:
                t_sol[n_sol] = res
                n_sol += 1
